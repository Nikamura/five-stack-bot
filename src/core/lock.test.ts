...
describe("party mode override", () => {
  const stacks = [5, 3, 2];

  function tallyFromYes(slot: number, ids: number[]): ReturnType<typeof tallySlots> {
    const roster = new Set(ids);
    return tallySlots({
      slots: [slot],
      votes: ids.map((u, i) => vote(u, slot, "yes", i + 1)),
      rosterIds: roster,
      skipIds: new Set(),
      fillerIds: new Set(),
    });
  ...
  it("expands a 5-stack lock to 6-stack when 6 committed are at the slot", () => {
    const tallies = tallyFromYes(1080, [1, 2, 3, 4, 5, 6]);
    const base = evaluateLock({ tallies, validStacks: stacks });
    assert.equal(base.size, 5);
    assert.deepEqual(base.core, [1, 2, 3, 4, 5]);
    assert.deepEqual(base.alternates, [6]);
    const party = applyPartyModeOverride({
      lock: base,
      tallies,
      partyMode: true,
    });
    assert.equal(party.size, PARTY_MODE_SIZE);
    assert.deepEqual(party.core, [1, 2, 3, 4, 5, 6]);
    assert.deepEqual(party.alternates, []);
  });

  it("counts a filler ✅ toward party-mode eligibility", () => {
    const tallies = tallySlots({
      ...
      votes: [
        ...
        vote(4, 1080, "yes", 4),
        vote(5, 1080, "yes", 5),
        vote(6, 1080, "yes", 6), // filler
      ],
      rosterIds: new Set([1, 2, 3, 4, 5, 6]),
      skipIds: new Set(),
      fillerIds: new Set([6]),
    });
    assert.equal(partyModeEligibleAtSlot(tallies[0]!), 6);
    const base = evaluateLock({ tallies, validStacks: stacks });
    const party = applyPartyModeOverride({
      lock: base,
      tallies,
  ...
  it("does NOT count a non-filler 🤷 toward party-mode eligibility", () => {
    // 5 ✅ + 1 🤷 (non-filler): the 🤷 isn't seated as alternate, so eligible < 6.
    const tallies = tallySlots({
      slots: [1080],
      votes: [
        vote(1, 1080, "yes", 1),
    ...
    assert.equal(partyModeEligibleAtSlot(tallies[0]!), 5);
    const base = evaluateLock({ tallies, validStacks: stacks });
    const party = applyPartyModeOverride({
      lock: base,
      tallies,
    ...
    // Eligible < 6 → override falls through, normal 5-stack survives.
    assert.equal(party.size, 5);
    assert.deepEqual(party.core, [1, 2, 3, 4, 5]);
  });

  it("is a no-op when partyMode is off", () => {
    const tallies = tallyFromYes(1080, [1, 2, 3, 4, 5, 6, 7]);
    const base = evaluateLock({ tallies, validStacks: stacks });
    const party = applyPartyModeOverride({
      lock: base,
      tallies,
    ...
    assert.deepEqual(party, base);
  });

  it("is a no-op when there is no lock to expand", () => {
    const tallies = tallySlots({
      slots: [1080],
      votes: [],
    ...
    const empty = { slot: null, size: null, core: [], alternates: [] };
    const party = applyPartyModeOverride({
      lock: empty,
      tallies,
      partyMode: true,
    });
    assert.deepEqual(party, empty);
  });
});
